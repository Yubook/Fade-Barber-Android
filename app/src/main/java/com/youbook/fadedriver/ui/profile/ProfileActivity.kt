...
import com.youbook.fadedriver.databinding.ActivityProfileBinding
...
import com.youbook.fadedriver.utils.*
import com.youbook.fadedriver.utils.Utils.handleApiError
...
import com.youbook.fadedriver.utils.Utils.visible
...
//                    params["state"] = selectedStateID
    ...
    private fun showSingleImage(uri: Uri, imageFor: String) {
        ...
        when (imageFor) {
            driverImage -> {
                mCurrentPhotoPath = com.youbook.fadedriver.utils.FileUtil.getPath(context, uri)
                Glide.with(context).load(mCurrentPhotoPath).into(binding.ivUserProfile)
            }

            driverIdPhoto -> {
                barberIdPhotoPath = com.youbook.fadedriver.utils.FileUtil.getPath(context, uri)
                cropImage(uri)
                //Glide.with(context).load(barberIdPhotoPath).into(binding.ivBarberIdPhoto)
            }
            driverProofPhoto -> {
                barberProofPhotoPath = com.youbook.fadedriver.utils.FileUtil.getPath(context, uri)
                cropImage(uri)
